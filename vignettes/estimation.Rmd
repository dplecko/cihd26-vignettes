...
```{r or-xgb}
xgb_y_xz <- cv_xgb(data.table(x = x, z), y) # Line 1
mu1 <- pred_xgb(xgb_y_xz, data.table(x = x, z), do_x = 1, X = "x") # Line 2
mu0 <- pred_xgb(xgb_y_xz, data.table(x = x, z), do_x = 0, X = "x") # Line 3
...
ATE_aipw <- mean(x * (y - mu1) / pi + mu1 - ((1 - x) * (y - mu0) / (1 - pi) + mu0))
